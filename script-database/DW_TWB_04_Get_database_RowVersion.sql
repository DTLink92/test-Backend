...
AS
	...
	SELECT Distinct a.[accidentid] as id
		  ,a.[nameAccidentado] as name_accidentado
		  ,a.[date_accident] as date
		  ,a.[place_accident] as place
		  ,a.[gravity_accident] as gravity
		  ,a.[description_accident] as description
		  ,at.[name_type_accident] as type
	FROM [dbo].[Accident] a
	inner join [dbo].[AccidentType] at ON a.accidentTypeId = at.accidentTypeId
	WHERE a.[Rowversion] > CONVERT(ROWVERSION, @LastRowVersionID)
	AND a.[Rowversion] <= CONVERT(ROWVERSION, @CurrentDBTS)
...
AS
	...
	SELECT Distinct a.[id] as area_id
		  ...
		  ,e.[gender]
	FROM [dbo].[Employee] e
	INNER JOIN [dbo].[Project_Contract] pc ON pc.Employeeid = e.employeeid
	INNER JOIN [dbo].[Position] p ON p.id = pc.positionid
	INNER JOIN [dbo].[Area] a ON a.id = p.area_id
	INNER JOIN [dbo].[Employee_Accident] ea ON ea.id_employee = e.employeeid
	INNER JOIN [dbo].[Accident] ac ON ac.accidentId = ea.id_accident
